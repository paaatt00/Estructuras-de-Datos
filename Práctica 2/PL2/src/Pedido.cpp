...
Pedido::Pedido(string descripcion, string direccion, int tiempo)
{
    descripcionArt = descripcion;
    direccionEntrega = direccion;
    tiempoPrepEnvio = tiempo;
...
string Pedido::imprimirPeque()
{
    string i = to_string(tiempoPrepEnvio);
    string pedido = descripcionArt + "//" + direccionEntrega + "//" + i;
    return pedido;
}
...
int Pedido::setTiempoPrepEnvio(int tiempo)
{
    tiempoPrepEnvio = tiempo;
}

void Pedido::socorroYaNoSeQueHacer(Pedido p)
{
    p.reducirTiempoPrepEnvio();
}
